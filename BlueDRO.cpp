...
void BlueDRO::bleRXCallback(int32_t characteristicId, uint8_t data[], uint16_t dataLength)
{
  ...
  {
    case BlueDRO::numeratorCharacteristicId:
      Serial.println("Setting numerator");
      break;

    case BlueDRO::denominatorCharacteristicId:
      Serial.println("Setting denominator");
      break;

    case BlueDRO::diameterModeCharacteristicId:
      Serial.println("Changing to diameter mode");
      break;

    case BlueDRO::reverseCharacteristicUuid:
      Serial.println("Reversing count direction");
      break;

    case BlueDRO::setPositionCharacteristicUuid:
...
bool BlueDRO::begin(bool reset)
{
  ...
  numeratorCharacteristicId    = _gatt.addCharacteristic(numeratorCharacteristicUuid,    GATT_CHARS_PROPERTIES_READ | GATT_CHARS_PROPERTIES_WRITE,  4, 4, BLE_DATATYPE_INTEGER, "Scaling numerator", &scalingFactorPresentationFormat);
  denominatorCharacteristicId  = _gatt.addCharacteristic(denominatorCharacteristicUuid,  GATT_CHARS_PROPERTIES_READ | GATT_CHARS_PROPERTIES_WRITE,  4, 4, BLE_DATATYPE_INTEGER, "Scaling denominator", &scalingFactorPresentationFormat);
  ...
  _ble.setBleGattRxCallback(1, bleRXCallback);
  _ble.setBleGattRxCallback(2, bleRXCallback);
  _ble.setBleGattRxCallback(3, bleRXCallback);
  _ble.setBleGattRxCallback(4, bleRXCallback);
  _ble.setBleGattRxCallback(5, bleRXCallback);
